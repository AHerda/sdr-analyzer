...
def create_plot(filename, data, title, subtitle, outliers_removed=False):
    ...
    for metric, color, label in zip(metrics, colors, labels):
        ax.plot(data['datetime'], data[metric], label=label, color=color, alpha=0.7, linewidth=0.9)
    ...
    plt.savefig(f'plots3/{filename}{suffix}.png')
    plt.close()
...
def create_heatmap(filename):
    ...
    for data_array, name in [(avgs, "avgs"), (mins, "mins"), (maxs, "maxs")]:
        ...
        plt.colorbar()
        plt.savefig(f'plots3/{filename[5:-4]}_{name}.png')
